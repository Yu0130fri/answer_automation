...
この実装では `python-dotenv` が優先的に使われます。もし未インストールの場合は内蔵の簡易ローダで読み込みを行います。推奨: `pip install -r requirements.txt` を実行して `python-dotenv` をインストールしてください。

`.env` のテンプレートは `.env.sample` に用意しています。これをコピーして `.env` を作成してください。
### 途中停止 / 再開
